...
from GraphCLIP.data.load import load_data  # noqa
from GraphCLIP.models import GraphCLIP  # noqa

add_safe_globals([DataEdgeAttr, DataTensorAttr, GlobalStorage, SparseTensor])

DATASET_NAME = "ogbn-arxiv"
CSV_PATH = "/home/xyx/GraphTranslator/data/arxiv/summary_embeddings_random100.csv"
NUM_SAMPLES = 100
NUM_HOPS = 1
OUTPUT_PATH = Path("graphclip_embeddings_test.pt")
...
def pad_sequences(records):
    ...
    for node_id, emb in records:
        ...
        if pad_len > 0:
            emb = torch.cat([emb, torch.zeros(pad_len, dim)], dim=0)
...
def main():
    ...
    print("Loading CSV with python engine...")
    df = pd.read_csv(
        CSV_PATH,
        engine="python",
        quotechar='"',
    ...
    with torch.no_grad():
        for idx, row in enumerate(df.itertuples(), start=1):
            ...
            _, _, node_embs, node_batch = model.graph_model(
                graph.x, graph.pe, graph.edge_index,
                graph.batch, graph.root_n_index,
                return_node_embeddings=True
            )

            node_embs = node_embs[node_batch == 0].cpu()
            raw_records.append((node_id, node_embs))

            if idx % 10 == 0 or idx == len(df):
                print(f"Processed {idx}/{len(df)} nodes...")

    padded = pad_sequences(raw_records)
    torch.save(padded, OUTPUT_PATH)
    print(f"Saved test seq embeddings to {OUTPUT_PATH}")
